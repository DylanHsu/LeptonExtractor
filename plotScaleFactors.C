...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void scale_factors(string plots_dir, string root_dir, string basename_config, int width=800, int height=1200, double xmin=0, double xmax=0, double ymin=0, double ymax=0) {
  ...
  while(getline(config_stream, line)) {
    ...
    for(unsigned i=(xRange ? h_eff_data->GetXaxis()->FindBin(xmin+.001):1); i<(xRange ? h_eff_data->GetXaxis()->FindBin(xmax-.001) : h_eff_data->GetXaxis()->GetNbins()); i++) {
      lines[nLines]=new TLine(
        h_eff_data->GetXaxis()->GetBinUpEdge(i),   
        yRange? h_eff_data->GetYaxis()->GetBinUpEdge(h_eff_data->GetYaxis()->FindBin(ymin+.001)-1) : h_eff_data->GetYaxis()->GetBinUpEdge(0),
        h_eff_data->GetXaxis()->GetBinUpEdge(i),
        yRange? h_eff_data->GetYaxis()->GetBinUpEdge(h_eff_data->GetYaxis()->FindBin(ymax-.001)) : h_eff_data->GetYaxis()->GetBinUpEdge(h_eff_data->GetYaxis()->GetNbins())
      ); lines[nLines]->SetLineStyle(kDashed); nLines++;
    }
    for(unsigned j=(yRange ? h_eff_data->GetYaxis()->FindBin(ymin+.001):1); j<(yRange ? h_eff_data->GetYaxis()->FindBin(ymax-.001) : h_eff_data->GetYaxis()->GetNbins()); j++) {
      lines[nLines]=new TLine(
        xRange? h_eff_data->GetXaxis()->GetBinUpEdge(h_eff_data->GetXaxis()->FindBin(xmin+.001)-1) : h_eff_data->GetXaxis()->GetBinUpEdge(0),
        h_eff_data->GetYaxis()->GetBinUpEdge(j),   
        xRange? h_eff_data->GetXaxis()->GetBinUpEdge(h_eff_data->GetXaxis()->FindBin(xmax-.001)) : h_eff_data->GetXaxis()->GetBinUpEdge(h_eff_data->GetXaxis()->GetNbins()),
        h_eff_data->GetYaxis()->GetBinUpEdge(j)
      ); lines[nLines]->SetLineStyle(kDashed); nLines++;
    }
    ...
    for(unsigned i=0;i<5;i++) {
      ...
      if(flavor=="Muon") gStyle->SetPaintTextFormat("4.2f"); else gStyle->SetPaintTextFormat("4.3f");
      if(flavor=="Muon") theHist[i]->Draw("TEXT45 COLZ"); else theHist[i]->Draw("TEXT COLZ");
      //canvas[i]->SetLogy();
      canvas[i]->Update();
      ...
      if(xmin!=xmax) theHist[i]->GetXaxis()->SetRangeUser(xmin,xmax-.001);
      if(ymin!=ymax) theHist[i]->GetYaxis()->SetRangeUser(ymin,ymax-.001);
      theHist[i]->GetYaxis()->SetMoreLogLabels();
      theHist[i]->SetMarkerSize(1.2);
...
void systematics(string methods_config, string basename_config) {
  ...
  while(getline(config_stream, line)) {
    ...
    int i0=1;
    for(int i = i0; i <= TMath::Min(i0-1+maxslices, h_nominal_sf->GetNbinsX()); i++) { // loop over each eta slice
      TGraphAsymmErrors *etaslice = new TGraphAsymmErrors( h_nominal_sf->GetNbinsY() );
      for(int j = 1; j <= h_nominal_sf->GetNbinsY(); j++) { // fill the pT points
      ...
      char name[128];
      sprintf(name, "etaslice%d", i-i0+1); 
      etaslice->SetName(name);
      etaslice->SetTitle((selection+" "+flavor+" scalefactors ( -#eta slices )").c_str() );
      etaslice->SetMarkerColor(marker_colors[i-i0]);
      etaslice->SetMarkerStyle(marker_styles[i-i0]);
      etaslice->SetLineColor(marker_colors[i-i0]);
      etaslice->SetLineWidth(2);
      etaslice->SetMinimum(0.6);
      etaslice->SetMaximum(1.5);
    ...
    TLegend *legend_etaslices = new TLegend(.75,.65,.93,.88);
    legend_etaslices->SetFillColor(0);
    for(int i = i0; i <= TMath::Min(i0-1+maxslices, h_nominal_sf->GetNbinsX()); i++) { // loop over each eta slice
      if(i==i0)  ( (TGraphAsymmErrors*) etaslices->At(i-i0)) ->Draw("AP");
      else       ( (TGraphAsymmErrors*) etaslices->At(i-i0)) ->Draw("P SAME");
      char legend_label[128];
      sprintf(legend_label, "%.4f < #eta < %.4f", h_nominal_sf->GetXaxis()->GetBinLowEdge(i), h_nominal_sf->GetXaxis()->GetBinUpEdge(i)); 
      legend_etaslices->AddEntry(( (TGraphAsymmErrors*) etaslices->At(i-i0)), legend_label, "lp");
    }
    ( (TGraphAsymmErrors*) etaslices->At(0))->GetXaxis()->SetRangeUser(
      h_nominal_sf->GetYaxis()->GetBinLowEdge(1),
      h_nominal_sf->GetYaxis()->GetBinUpEdge( h_nominal_sf->GetNbinsY()-1 )
    );
