...
void findTransferFunction(
  ...
  vFreqGenRe.reserve(nbins);
  vFreqGenIm.reserve(nbins);
  vFreqRecoRe.reserve(nbins);
  vFreqRecoIm.reserve(nbins);

  // gen,reco distributions in the frequency domain
  TH1D *hFreqGen=0, *hFreqReco=0; TVirtualFFT *fft;
  ...
  // shift for plotting
  TH1D *hFreqGenScaled = new TH1D("hFreqGenScaled","hFreqGenScaled",nbins,-.5*(1.-1./nbins),.5*(1.+1./nbins));
  TH1D *hFreqRecoScaled = new TH1D("hFreqRecoScaled","hFreqRecoScaled",nbins,-.5*(1.-1./nbins),.5*(1.+1./nbins));
  TH1D *hResFuncShifted = new TH1D("hResFuncShifted","hResFuncShifted",nbins,(fitMassLo-fitMassHi)/2.,(fitMassHi-fitMassLo)/2.);
  for(int i=1; i<=nbins; i++) { 
    int j;
    if(i<=nbins/2+1)   j=i+nbins/2-1;
    else             j=i-nbins/2-1;
    hResFuncShifted->SetBinContent(j, resolutionFunction->GetBinContent(i));
    //if(i<=nbins/2)  j=i+nbins/2-1; 
    //else            j=i-nbins/2;
    //if(i==1) continue; // start at 2 to ignore the DC component.
